...
class MaxelerFPGA(Platform.Platform):
  ...
  def __init__(self,platform_directory_string="Platforms/MaxelerFPGA/maxeler_code/build",root_directory_string=None,ssh_alias="",remote=False,hostname="",shell_vars={},board="max3",boardid=":0"):    
    ...
    ssh_cmd = ["ssh","%s"%self.ssh_alias,"source",".profile;"]
    if(board=="max4"): ssh_cmd += ["bash","-c","\"","maxorch -r 192.168.0.1 -c reserve -i %s -t MAIA*1 > /dev/null"%self.boardid,"\""]
  ...
  def __del__(self):
    if(self.board=="max4"): 
	ssh_cmd = ["ssh","%s"%self.ssh_alias,"bash","-c","\"","maxorch -r 192.168.0.1 -c unreserve -i %s > /dev/null"%self.boardid,"\""]
    	output = subprocess.check_output(ssh_cmd)
