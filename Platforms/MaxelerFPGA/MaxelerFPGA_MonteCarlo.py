...
class MaxelerFPGA_MonteCarlo(MulticoreCPU_MonteCarlo.MulticoreCPU_MonteCarlo):
  def __init__(self,derivative,paths,platform,points=4096,reduce_underlyings=True,c_slow=False):
  ...
  def generate_kernel(self,overide=True):
    ...
    if(self.c_slow):
      ...
      output_list.append("DFEVar p = chain.addCounter(this.instance_paths,1).cast(dfeUInt(32));")
      output_list.append("DFEVar pp = chain.addCounter(this.path_points+1,1).cast(dfeUInt(32));") #Path Points is the outer loop so as to implement a C-Slow architecture
      output_list.append("chain.addCounter(this.delay,1);")
	...
	output_list.append("CombinedTauswortheRNG %s_%d_x = new CombinedTauswortheRNG(this,this.delay,input_array[%d*4],input_array[%d*4+1],input_array[%d*4+2],input_array[%d*4+3]);"%(u.name,index,index,index,index,index));
	output_list.append("CombinedTauswortheRNG %s_%d_y = new CombinedTauswortheRNG(this,this.delay,input_array[%d*4+4],input_array[%d*4+5],input_array[%d*4+6],input_array[%d*4+7]);"%(u.name,index,index,index,index,index));
  ...
  def generate_hw_builder(self):
    ...
    else: output_list.append("private static int delay = %d;"%self.solver_metadata["delay"])
