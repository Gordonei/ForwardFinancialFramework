...
class OpenCLXilinxFPGA_MonteCarlo(OpenCLAlteraFPGA_MonteCarlo.OpenCLAlteraFPGA_MonteCarlo):
	...
	def compile(self,override=True,cleanup=True,debug=False):
		...
		results += [subprocess.check_output(["cp","%s/%s_build/impl/host/%s/%s_build.exe"%(directory_string,self.output_file_name,self.output_file_name),arch,"%s/%s"%(directory_string,self.output_file_name)])]
