...
proc setup_microblaze {} {
    ...
    create_bd_cell -type ip -vlnv xilinx.com:ip:microblaze:$MB_VER $MB_INST
...
############################################################
proc make_external_interface {intf} {
    # {{{
    ...
    foreach property [list_property $intf CONFIG.*] {
        if {$property == "CONFIG.FREQ_HZ"} {
            # do not set FREQ_HZ since it may mismatch
            set_property -quiet $property "" $port
        } else {
...
proc connect_to_vcc {pin} {
    ...
    connect_bd_net $pin [get_bd_pins /$vcc/const]
    # }}}
}
...
proc get_vcc_instance {} {
    ...
    if {$vcc == ""} {
        set vcc [create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.0 $VCC_INST]
    }
