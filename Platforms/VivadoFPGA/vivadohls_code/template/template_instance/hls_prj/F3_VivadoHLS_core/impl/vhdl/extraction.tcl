...
#----------------------------------------------------------------------------
proc compile_reports_rodin {top_module language device} {
    set result_file [open ${top_module}.result w]
    dump_report_rodin $top_module $result_file $device
    close $result_file
    ...
    dump_report_rodin $top_module stdout $device
}
...
proc extract_area_rodin { uti_file } {
    ...
    ################################################################################
    # SLICE pattern
    set slice_pattern {.*(?:Slice|CLB)\s+.+?(\d+).*}
    # LUT pattern
    set lut_pattern {.*(?:Slice|CLB) LUTs.+?(\d+).*}
    # FF pattern
    set ff_pattern {.*(?:Slice|CLB) Registers.+?(\d+).*}
    # DSP pattern
    set DSPs_pattern {.*DSPs.+?(\d+).*}
    # BRAM pattern
    set bram_pattern {.*RAMB18E1 only.+?(\d+).*}
    set bram36_pattern {.*RAMB36E1 only.+?(\d+).*}
    # FIFO pattern
    set fifo_pattern {.*FIFO18E1 only.+?(\d+).*}
    # LATCH pattern
...
proc extract_total_rodin { uti_file } {
    ...
    ################################################################################
    # SLICE pattern
    set slice_pattern {.*(?:Slice|CLB)\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+(\d+).*}
    # LUT pattern
    ...
    set return_value [list $slice_count $lut_count $ff_count $dsp_count $bram_count18]
...
#
# Opens the .sta file to extract the actual timing delay value
#----------------------------------------------------------------------------
proc extract_timing_rodin {file_name} {
    ...
    # pattern for parsing .sta file.
    set clk_pattern_sync_setup {.*Setup.*?Worst Slack +?(\S+)ns,.*}
    set clk_pattern_sync_hold {.*Hold.*?Worst Slack +?(\S+)ns,.*}
    set clk_pattern_sync_pw {.*PW.*?Worst Slack +?(\S+)ns,.*}
    if {[regexp ${clk_pattern_sync_setup} $report_buf match setup_slack] && [regexp ${clk_pattern_sync_pw} $report_buf match pw_slack]}  {
...
proc dump_report_rodin { module out device} {

    #set fl [open ./report/${module}_post_route.sta r]
    #set fl [open ./report/${module}_par_timing.rpt r]
    set fl [open ./report/${module}_timing_routed.rpt r]
    set report_buf [read -nonewline $fl]
    close $fl
    ...
    #set area_results [extract_area_rodin ./report/${module}_post_route.uti]
    #set area_results [extract_area_rodin ./report/${module}_par_area.rpt]
