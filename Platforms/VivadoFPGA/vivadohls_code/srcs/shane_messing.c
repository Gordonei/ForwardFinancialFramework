...
FP_t temp_test_array[K_DIMENSION];
FP_t temp_test_arry_sqrd[K_DIMENSION];
...
#pragma HLS INTERFACE ap_stable port=thread_arg
	...
	//struct thread_data* temp_data;
	//temp_data = (struct thread_data*) thread_arg;
	underlying_attributes u_a_0 = thread_arg->u_a_0;
	underlying_variables u_v_0;
	option_attributes o_a_0 = thread_arg->o_a_0;
	option_variables o_v_0;
	...
	int j,k;
	//**Thread Calculation Loop**
	outer_loop: for(k=0;k<K_DIMENSION;++k){
	...
	#pragma AP PIPELINE
		//**Running the path**
		inner_loop: for(j=0;j<J_DIMENSION;++j){
		...
		#pragma HLS unroll factor=10
			...
			//temp_test_array[K_DIMENSION]
			//temp_test_arry_sqrd[K_DIMENSION]
			...
			if( j== (J_DIMENSION-1) )
			{
				...
				//temp_total_0 += o_v_0.value;
				//temp_total_sqrd_0 += pow(o_v_0.value,2);
				temp_test_array[k] = o_v_0.value;
				temp_test_arry_sqrd[k] = pow(o_v_0.value,2);
	...
	//Perform the reduction tree on the array here

	result_loop: for(k=0; k<K_DIMENSION; k++)
	{
		#pragma HLS unroll factor=25
		temp_total_0 += temp_test_array[k];
		temp_total_sqrd_0 += temp_test_arry_sqrd[k];
	}

	//**Returning Result**
	thread_arg->thread_result = temp_total_0;
	thread_arg->thread_result_sqrd = temp_total_sqrd_0;
	}
