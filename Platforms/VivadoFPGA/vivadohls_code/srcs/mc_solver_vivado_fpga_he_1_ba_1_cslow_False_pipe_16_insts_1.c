...
static FP_t barrier_option_0_time_period;
static FP_t barrier_option_0_call;
static FP_t barrier_option_0_strike_price;
static FP_t barrier_option_0_points;
static FP_t barrier_option_0_barrier;
static FP_t barrier_option_0_out;
static FP_t barrier_option_0_down;
static int paths;
...
typedef struct{
	...
	barrier_option_attributes o_a_0;
	} kernel_data;
...
void vivado_activity_thread(volatile int *a,standard_underlying_attributes *kernel_u_a_0,standard_derivative_attributes *kernel_o_a_0,rng_state_t *seed_0,unsigned int thread_result_0){
	...
	barrier_option_attributes o_a_0;
	barrier_option_variables *o_v_0;
	barrier_option_variables o_v_0_arr[1];
	o_v_0 = o_v_0_arr;

	//***Derivative Attributes Initialisation***
	barrier_option_derivative_init(kernel_o_a_0->time_period,kernel_o_a_0->call,kernel_o_a_0->strike_price,kernel_o_a_0->points,kernel_o_a_0->barrier,kernel_o_a_0->out,kernel_o_a_0->down,&o_a_0);
	...
	PATHSET_LOOP: for(p=0;p<PATHS;++p){
		...
		time_0 = u_v_0->time;
		barrier_option_derivative_path_init(o_v_0,&o_a_0);
		delta_time_0 = o_a_0.time_period/PATH_POINTS;

		//**Running the path**
		PATH_LOOP: for(pp=0;pp<(PATH_POINTS);++pp){
			barrier_option_derivative_path(spot_price_0,time_0,o_v_0,&o_a_0);
		...
		barrier_option_derivative_payoff(spot_price_0,o_v_0,&o_a_0);
...
void * multicore_montecarlo_activity_thread(void* thread_arg){
	...
	barrier_option_derivative_init(barrier_option_0_time_period,barrier_option_0_call,barrier_option_0_strike_price,barrier_option_0_points,barrier_option_0_barrier,barrier_option_0_out,barrier_option_0_down,&o_a_0);
...
int main(int argc,char* argv[]){
	...
	barrier_option_0_time_period = strtod(argv[18],NULL);
	barrier_option_0_call = strtod(argv[19],NULL);
	barrier_option_0_strike_price = strtod(argv[20],NULL);
	barrier_option_0_points = strtod(argv[21],NULL);
	barrier_option_0_barrier = strtod(argv[22],NULL);
	barrier_option_0_out = strtod(argv[23],NULL);
	barrier_option_0_down = strtod(argv[24],NULL);

	//**Calculating Discount Factor**
	discount_0_0 = exp(-heston_underlying_0_rfir*barrier_option_0_time_period);
