...
class OpenCLGPU_MonteCarlo(MulticoreCPU_MonteCarlo.MulticoreCPU_MonteCarlo):
  def __init__(self,derivative,paths,platform,reduce_underlyings=True):
    ...
    self.platform_name = "AMD"
    self.device_type = pyopencl.device_type.CPU
    
    self.platform = None
    for p in pyopencl.get_platforms(self.device_type):
      if(self.platform_name in str(p)): self.platform = p
      
    self.device = platform.get_devices(pyopencl.device_type.CPU)[0]
  ...
  def compile(self,override=True,cleanup=True):
    ...
    self.program = pyopencl.Program(self.context,self.generate_kernel())
    self.program.build()
