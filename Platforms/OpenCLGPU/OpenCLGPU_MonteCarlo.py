...
class OpenCLGPU_MonteCarlo(MulticoreCPU_MonteCarlo.MulticoreCPU_MonteCarlo):
  def __init__(self,derivative,paths,platform,reduce_underlyings=True,chunk_paths=1000):
    ...
    self.solver_metadata["chunk_paths"] = chunk_paths
  ...
  def generate_activity_thread(self):
   ...
   ###Outputing the Build Log
    ...
    for index,u in enumerate(self.underlying):
        ...
        output_list.append("cl_mem u_v_%d_buff = clCreateBuffer(context, CL_MEM_READ_ONLY,chunk_paths*sizeof(mwc64x_state_t),NULL,NULL);" % (index))
        if("AMD" in self.platform.platform_name):
          output_list.append("mwc64x_state_t *u_v_%d = (mwc64x_state_t*) malloc(chunk_paths*sizeof(mwc64x_state_t));" % (index)) #Mallocs, here?
          output_list.append("cl_mem cpu_u_v_%d_buff = clCreateBuffer(cpu_context, CL_MEM_WRITE_ONLY,chunk_paths*sizeof(mwc64x_state_t),NULL,NULL);" % (index))
    ...
    for index,d in enumerate(self.derivative):
        ...
        output_list.append("%s_variables *o_v_%d = (%s_variables*) malloc(chunk_paths*sizeof(%s_variables));" % (d.name,index,d.name,d.name)) #Mallocs, here?
    ...
    output_list.append("int chunks = temp_data->thread_paths/chunk_paths;")
    ...
    for d_index,d in enumerate(self.derivative):
        output_list.append("clEnqueueReadBuffer(command_queue, o_v_%d_buff, CL_TRUE, 0, chunk_paths * sizeof(%s_variables),o_v_%d, 0, NULL, NULL);"%(d_index,d.name,d_index))
	...
	output_list.append("clReleaseMemObject(o_a_%d_buff);" % (index))
        output_list.append("clReleaseMemObject(o_v_%d_buff);" % (index))
  ...
  def generate_kernel(self):
    ...
    for index,u in enumerate(self.underlying): 
        ...
        output_list.append("FP_t spot_price_%d = u_a_%d[i].current_price*exp(u_v_%d[i].gamma);"%(index,index,index))
        output_list.append("FP_t time_%d = u_v_%d[i].time;"%(index,index))"""
