...
class OpenCLGPU_MonteCarlo(MulticoreCPU_MonteCarlo.MulticoreCPU_MonteCarlo):
  def __init__(self,derivative,paths,platform,reduce_underlyings=True,kernel_path_max=10):
    ...
    self.min_paths = self.kernel_loops*self.chunk_paths #self.kernel_loops*self.solver_metadata["local_work_items"]*self.chunk_paths #i.e. the number of paths in one chunk
  ...
  def generate_kernel(self):
	...
	output_list.append("temp_u_v_%d.rng_state = seed_%d[i];"%(index,index))
        ...
        output_list.append("MWC64X_SeedStreams(&(temp_u_v_%d.rng_state),(local_chunk_size*local_chunk_number+ local_seed),%d*%d*4096*2*i);"%(index,index,self.kernel_loops))
  ...
  def generate_cpu_seed_kernel(self):
	...
	output_list.append("mwc64x_state_t temp_seed_%d;"%(i))
        output_list.append("MWC64X_SeedStreams(&temp_seed_%d,(local_chunk_size*local_chunk_number + local_seed),%d*%d*4096*2*i);"%(index,index,self.kernel_loops))
