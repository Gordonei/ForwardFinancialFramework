...
class OpenCLGPU_MonteCarlo(MulticoreCPU_MonteCarlo.MulticoreCPU_MonteCarlo):
  ...
  def compile(self,override=True,cleanup=True):
    result = MulticoreCPU_MonteCarlo.MulticoreCPU_MonteCarlo.compile(self,overide) #Compiling Host C Code
      
    os.chdir("..")
    os.chdir(self.platform.platform_directory())
    self.program = pyopencl.Program(self.platform.context,self.kernel_code_string).build() #Creating OpenCL program based upon Kernel
    
    binary_kernel = program.get_info(pyopencl.program_info.BINARIES)[0] #Getting the binary code for the OpenCL code
    binary_kernel_file = open("%s.clbin"%self.output_file,"wb") #Writing the binary code to a file to be read by the Host C Code
