...
for filename in sys.argv[1:]:
    ...
    accuracy_model_data = numpy.array([accuracy_model(path) for path in verification_paths])
    latency_model_data = numpy.array([latency_model(path) for path in verification_paths])
    
    latency_model_error = numpy.abs((latency_model_data-latency_verification_data)/latency_verification_data)
    accuracy_model_error = numpy.abs((accuracy_model_data-accuracy_verification_data)/accuracy_verification_data)
...
plt.ylabel(r"Relative Model Error (%)",size=20)
