...
for filename in sys.argv[1:]:
    ...
    for degree in range(max_degree):
        ...
        accuracy_model_data = accuracy_model(verification_path) #numpy.array([accuracy_model(p) for p in benchmark_paths])
        latency_model_data = latency_model(verification_path) #numpy.array([latency_model(p) for p in benchmark_paths])
        
        verification_accuracy_model = generate_accuracy_model(accuracy_verification_data,verification_paths,degree)
        verification_latency_model = generate_latency_model(latency_verification_data,verification_paths,degree)
        
        accuracy_verification_model_data = verification_accuracy_model(verification_path)
        latency_verification_model_data = verification_latency_model(verification_path)
        
        accuracy_degree_error.append(100.0*numpy.abs((accuracy_verification_model_data-accuracy_model_data)/accuracy_verification_model_data))
        latency_degree_error.append(100.0*numpy.abs((latency_verification_model_data-latency_model_data)/latency_verification_model_data))
