...
var task_max real;
var A_n[PT] binary; # Allocation proportion binary matrix - used to indicate that a platform is in use.
...
#	forall <p> in PLATFORMS:
#                sum <t> in TASKS: 
#			(if (A[p,t]>0) then 1.0 else 0.0 end)*LATENCY_SETUP[p,t] <= task_max_setup;
		
		#if (A[p,t]>0) then 1.0 else 0.0 end 
		#A_n[p,t]*LATENCY_SETUP[p,t] <= task_max_setup;
		#A_n[p,t] == ceil(A[p,t]);				
